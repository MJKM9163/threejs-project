...
export const EnemyFighter = ({ args, position, mPos, rotation, num }) => {
  ...
  useFrame(() => {
    ...
    if (boundingDetect()) {
      ...
      if (launch === false) {
        ...
        const plus = new Vector3(0, 1500, 0);
        const TPos = new Vector3(X, 0, Z);
        //const TPos = new Vector3(-1000, 0, 0);
        ...
        // const test = boundingStore.getState().enemyNum;
        // boundingStore.getState().enemyNum = [...test, 3];
        // console.log(test);

        //boundingStore.setState({ enemyNum: [...add, PosArray] });
  ...
  return (
    <group>
      ...
      {/* <MemoExplosive controll={missilesRef} api={missilesApi} pos={mPos} /> */}
    </group>
  );
};
